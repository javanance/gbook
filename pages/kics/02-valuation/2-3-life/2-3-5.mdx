...
order: 7
---
import {Button, Accordion, AccordionTab, Card, Panel, TabView, TabPanel, OverlayPanel, CenterImg, SubText, SCard, Math,Fieldset}  from '../../../../components/prime'
...
손실조정은 재보험 거래상대방의 장래 부도확률, 부도에 따른 손실 및 회수 금액 등을 감안하여 다음 계산식으로 측정한다.
 ...
 <Math>
  손실조정 =
 <center>
 $\displaystyle\sum_i^n (PD_i \times$ 부도시손실액$_i)$
 </center>
 <SCard>
  - $i$ =  평가시점부터 만기까지의 각 시점.
  - $n$ =  만기
...
<Fieldset legend = "손실조정 측정방법" toggleable>
...
### (3) 간편법
 ...
 $CE_{조정후} =$
 <center>
  $CE_{조정전} - max($손실조정률(%) $\times CE_{조정전}, 0)$
 </center>
 <SCard>
...
<Fieldset legend = "간편법을 이용한 손실조정 반영" toggleable>

- ① 손실조정 반영 전 재보험자산의 장래 현금흐름 현가는 “(1)”의 구분기준 별로 산출하여야 한다. 다만, “(1)”의 구분기준 별로 장래 현금흐름을 산출하기 어려운 경우, 합리적인 방법에 따라 배분하여 사용할 수 있다.
- ② 재보험 거래상대방의 부도확률은 <OverlayDemo id="452_나" />(1) 에서 정한 신용평가기관의 장기평균(10년 이상) 1년 부도율을 적용한다.
